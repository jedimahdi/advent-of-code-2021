...
initialPosition = Position { horizontalPosition = 0, depth = 0 }
...
runCommand (Forward x) p = p { horizontalPosition = horizontalPosition p + x }
runCommand (Down x) p    = p { depth = depth p + x }
runCommand (Up x) p      = p { depth = depth p - x }
